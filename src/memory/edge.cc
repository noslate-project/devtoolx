...
std::string Edge::GetType(int id, bool source) {
  int edge_field_length = static_cast<int>(this->parser_->edge_field_length);
  ...
  if(edge_source_index / edge_field_length >= static_cast<int>(this->parser_->edge_count)) {
    Nan::ThrowTypeError(Nan::New<v8::String>("edge id larger than edges.length!").ToLocalChecked());
    return "error";
  }
  int type = static_cast<int>(this->parser_->edges[edge_source_index + this->parser_->edge_type_offset]);
  json types = this->parser_->edge_types;
  return types[type];
}
...
int Edge::GetTypeForInt(int id, bool source) {
  ...
  return static_cast<int>(this->parser_->edges[edge_source_index + this->parser_->edge_type_offset]);
}
...
std::string Edge::GetNameOrIndex(int id, bool source) {
  ...
  int type = static_cast<int>(this->parser_->edges[edge_source_index + this->parser_->edge_type_offset]);
  int name_or_index = static_cast<int>(this->parser_->edges[edge_source_index + this->parser_->edge_name_or_index_offset]);
  if(type == KELEMENT || type == KHIDDEN) {
    return "[" + std::to_string(name_or_index) + "]";
  } else {
    return this->parser_->strings[name_or_index];
  };
}
...
int Edge::GetTargetNode(int id, bool source) {
  ...
  int target_node_source_id = static_cast<int>(this->parser_->edges[edge_source_index + this->parser_->edge_to_node_offset]);
