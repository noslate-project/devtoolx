...
int Node::GetNodeId(int source) {
  int node_field_length = static_cast<int>(parser_->node_field_length);
...
int Node::GetAddress(int id, bool source) {
  ...
  if(node_source_index / node_field_length >= static_cast<int>(parser_->node_count)) {
    Nan::ThrowTypeError(Nan::New<v8::String>("node id larger than nodes.length!").ToLocalChecked());
    return -1;
  }
  return static_cast<int>(parser_->nodes[node_source_index + parser_->node_address_offset]);
}
...
int* Node::GetEdges(int id, bool source) {
  ...
  if(node_ordinal_index >= static_cast<int>(parser_->node_count)) {
    Nan::ThrowTypeError(Nan::New<v8::String>("node id larger than nodes.length!").ToLocalChecked());
    return new int[0];
  }
  int first_edge_index = parser_->first_edge_indexes[node_ordinal_index];
  int next_first_edge_index = 0;
  if(node_ordinal_index + 1 >= static_cast<int>(parser_->node_count)) {
